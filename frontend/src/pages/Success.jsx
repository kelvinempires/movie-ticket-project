...
const Success = () => {
  ...
  const bookingNumber = `CINE-${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
  ...
  return (
    <div className="container mx-auto px-4 4 py-12">
<div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
<div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-6 text-center">
<FaCheckCircle className="text-5xl text-white mx-auto mb-4" />
<h1 className="text-3xl font-bold text-white mb-2">Booking Confirmed!</h1>
<p className="text-purple-100">Your tickets have been successfully booked</p>
</div>
    <div className="p-6">
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        ...
        />
        <div>
          <h2 className="text-2xl font-semibold text-gray-800 mb-2">{state.movie.title}</h2>
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <p className="font-medium">{state.selectedTime}</p>
            </div>
            <div>
              <p className="text-gray-600">Theater</p>
              <p className="font-medium">{state.selectedTheater}</p>
            </div>
            <div>
              <p className="text-gray-600">Seats</p>
              <p className="font-medium">{state.selectedSeats.join(', ')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200 pt-6 mb-6">
        ...
        <div className="bg-gray-50 p-4 rounded-md">
          <div className="flex justify-between mb-2">
            <span className="text-gray-600">Tickets ({state.selectedSeats.length})</span>
            <span className="font-medium">${(state.selectedSeats.length * state.movie.price).toFixed(2)}</span>
          </div>
          <div className="flex justify-between mb-2">
          ...
          <div className="flex justify-between border-t border-gray-200 pt-2 mt-2">
            ...
            <span className="text-purple-600 font-bold">
              ${(state.selectedSeats.length * state.movie.price + 2.5).toFixed(2)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200 pt-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Booking Reference</h3>
